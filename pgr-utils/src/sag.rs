...
pub fn read_aln_records(filename: &String, remove_self: bool) -> Result<Vec<MapIntervalRecord>, std::io::Error> {
...
pub fn get_aln_best( aln_recs: &Vec<[u32;8]> ) -> 
    FxHashMap<u32, (u32, u32, u32, u32)> {

    let mut aln_count = 
        FxHashMap::<u32, FxHashMap<(u32, u32), u32>>::default(); // id0 -> (id1, strand), count
...
pub fn get_sag_from_aln_records(
        ...
        contig: Option<u32>,
    ) -> (Vec<u32>, IntervalAlnGraph, FxHashMap<u32, (u32, u32, u32, u32)>) {
    ...
    .into_iter()
    .filter(|&&r| if let Some(the_contig) = contig {r[0] == the_contig} else {true}) 
        ...
        .map(|(&s1, path1)| {
            ...
            if aln_best.contains_key(&s1) {
                ...
                if path1.len() > path0.get(s0).unwrap().len() {
                    tag = 2_u32;
                } else if *count > 1 && {if let Some(the_contig) = contig {*s0 == the_contig} else {true}} {
                    tag = 1_u32;
                } else {
                    tag = 0_u32;
                }
