...
pub fn find_match_chain(matches: &Vec<MapIntervalRecord>, max_count:Option<u32>) -> Vec<MapIntervalRecord> {
    ...
    let mut match_for_pair = FxHashMap::<(u32, u32), Vec<&MapIntervalRecord>>::default();
    let max_count = max_count.unwrap_or(12);
    ...
    let out: Vec<MapIntervalRecord> = seqpair_count
        .iter()
        .filter(|(_, &count)| count >= 4)
        .flat_map(|((sid0, sid1), _count)| {
            //if *count < 4 {
            //    continue;
            //}
            let mut aln_g = DiGraphMap::<u32, u32>::new();
            let filtered_matches = match_for_pair.get(&(*sid0, *sid1)).unwrap();

            let edges: Vec<(u32, u32)> = (0..filtered_matches.len())
                .into_iter()
                .flat_map(|i| {
                    let mut j = i + 1;
                    let mut edges = Vec::<(u32, u32)>::new();
                    let mut d_count = FxHashSet::<u32>::default();
                    loop {
                        if j >= filtered_matches.len() || d_count.len() > max_count as usize {
                            break;
                        }
            ...
            let mut out_tmp = Vec::<MapIntervalRecord>::new();
...
pub fn rc_dna_seq(seq: Vec<u8>) -> Vec<u8> {
...
pub fn generate_deltas(seqdb0: &SeqDB, seqdb1: &SeqDB, m: MapIntervalRecord, k: u32) -> usize {
    let mut rev = false;
    if m[6] == 1 {
        //strand
        rev = true;
...
pub fn filter_chain_group(
    chain_groups: FxHashMap<(u32, u32, u32), Vec<[u32; 8]>>,
) -> Vec<[u32; 8]> {
    ...
    for (_k, v) in chain_list.iter() {
        ...
        // only use unique hits in each chain
...
pub fn map_seqs_with_db(seq0_id_length: Vec<[u32; 2]>, shmmrmap: &MapIntervals) -> Vec<[u32; 8]> {
    let rmv = seq0_id_length
        .into_par_iter()
        //.into_iter()
        .flat_map(|[sid, length]| {
            let mq = map_interval_query(&shmmrmap, sid as u32, 0, length as u32);
            let mq_filtered = find_match_chain(&mq, None);
    ...
    //aln_range: map from (sid0, sid1, chainlabel) -> chains (moved)
    let mut sid1_to_chains = FxHashMap::<(u32, u32, u32), Vec<[u32; 8]>>::default();
    rmv.into_iter().for_each(|v| {
        let key = (v[0], v[3], v[7]);
        sid1_to_chains.entry(key).or_insert_with(|| vec![]).push(v);
    });

    // uniq to the target seq
    let mut sid1_to_chains2 = FxHashMap::<u32, Vec<(u32, u32, Vec<[u32; 8]>)>>::default();
    sid1_to_chains.into_iter().for_each(|(k, v)| {
        sid1_to_chains2
            .entry(k.1)
            .or_insert_with(|| vec![])
            .push((k.0, k.2, v));
    ...
    fn filter_out(cov: &Vec<(u32, u32)>, r: (u32, u32)) -> bool {
        for c in cov {
            if c.0 <= r.0 && r.1 <= c.1 {
                return true;
            }
    ...
    sid1_to_chains2
        .iter_mut()
        .flat_map(|(_sid1, vs)| {
            vs.sort_unstable_by(|a, b| b.2.len().partial_cmp(&a.2.len()).unwrap());
            let mut cov = Vec::<(u32, u32)>::new();
            vs.iter()
                .flat_map(|v| {
                    let mut min = u32::MAX;
                    let mut max = u32::MIN;
                    let out =
                        v.2.iter()
                            //.filter(|&v| !filter_out(&cov, (v[4], v[5]))) // This makes the mapping from contig to reference unique, we need to relax this for trio phasing
                            .map(|v| {
                                if v[4] < min {
                                    min = v[4]
                                };
                    ...
                    //println!("{} {} {} {:?}", _sid1, v.0, v.1, cov);
                    out
                })
                .collect::<Vec<[u32; 8]>>()
            //vs.get(0).unwrap().2.to_owned()
        })
        .collect::<Vec<[u32; 8]>>()
}
...
pub fn map_seqs(
    ...
    let seq_id_length = (0..sdb0.seqs.len())
        .into_iter()
        .map(|sid| {
            let name = &sdb0.id2name[&(sid as u32)];
            let length = sdb0.lengths[name];
            [sid as u32, length as u32]
        })
        .collect::<Vec<[u32; 2]>>();

    let v = map_seqs_with_db(seq_id_length, &shmmrmap)
        .into_par_iter()
        .map(|m| {
