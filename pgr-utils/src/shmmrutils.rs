...
pub fn get_hpc_seq(seq0: &Vec<u8>) -> HPCSeq {
    ...
    while i < seq0len - 1 {
        i += 1;
        if i < seq0len - 1 && i > 1 && seq0[i - 2] == seq0[i] && seq0[i - 1] == seq0[i + 1] {
            i += 1;
            continue;
        }
        if seq0[i] != seq0[i - 1] {
...
pub fn get_seq_fragment(
    ...
    let b = rloc.start + bgn as usize;
    let e = rloc.start + end as usize;

    match strand {
        0 => mmap[b..e]
            .iter()
            .for_each(|c| seq.push(base_map[(c & 0b0011) as usize])),
        1 => mmap[b..e]
            .iter()
            .for_each(|c| seq.push(base_map[((c >> 4) & 0b0011) as usize])),
        _ => (),
    }
    seq
}
