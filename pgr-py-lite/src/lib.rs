...
impl SeqIndexDB {
    ...
    pub fn generate_smp_gfa(&self, min_count: usize, filepath: &str) -> PyResult<()> {
        ...
        let mut overlaps = FxHashMap::<((u64, u64, u8), (u64, u64, u8)), Vec<(u32, u8, u8)>>::default();
        ...
        out_file.write("# Sparse Genome Graph Generated By pgr-tk\n".as_bytes())?;
        out_file.write("H\tVN:Z:1.0".as_bytes())?;
        (&frag_id).into_iter().try_for_each(|(smp, id)| -> PyResult<()> {
            let hits = frag_map.get(&smp).unwrap();
            let ave_len =
                hits.iter().fold(0_u32, |len_sum, &s| len_sum + s.3 - s.2) / hits.len() as u32;
            let seg_line = format!("S\t{}\t*\tLN:i:{},SN:Z:{}_{}\n", id, ave_len, smp.0, smp.1);
        ...
        overlaps.into_iter().try_for_each(|(op, vs)| -> PyResult<()>{
